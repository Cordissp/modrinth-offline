<template>
	...
	>
		...
		<input v-model="loginUsername" type="text" placeholder="Username" class="bg-surface-0 border border-solid border-surface-15 rounded-md px-3 py-1.5 text-contrast focus:outline-none focus:border-brand" @keydown.enter="!loginDisabled && loginUsername && login()" />
		<ButtonStyled color="brand">
			<button color="primary" :disabled="loginDisabled || !loginUsername" @click="login()">
			...
			<div class="flex flex-col gap-2 px-2 pt-2">
				<template v-if="accounts.length > 0">
					...
					<ButtonStyled class="w-full">
						<button :disabled="loginDisabled || !loginUsername" @click="login()">
							<PlusIcon />
...
async function login() {
	...
	const loggedIn = await login_flow(loginUsername.value).catch(handleSevereError)
